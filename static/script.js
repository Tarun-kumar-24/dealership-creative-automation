...
//     let res = await fetch('/dealers/' + id);
//     let data = await res.json();
...
//     data.forEach(d => {
//         let el = document.createElement('div');
//         el.innerHTML = `<input type="checkbox" value="${d.name}"> ${d.name}`;
//         div.appendChild(el);
//     });
// }

// document.getElementById('account').onchange = loadDealers;

// async function generate() {
    
//     let file = document.getElementById('bg').files[0];

//     let checks = document.querySelectorAll('#dealers input:checked');
...
//     let useLogo = document.getElementById("useLogo").checked;
//     form.append("useLogo", useLogo);
